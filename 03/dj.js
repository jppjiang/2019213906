...
Dj.prototype = {
    ...
    addEdge:function(v1,v2,val) {
        if (v1 >= this.vexnum || v2 >= this.vexnum) {
            alert("点不存在");
        } else if (v1 == v2) {
            alert("请选择不同的点创建边");
        } else if (this.edge[v1][v2] != this.infinite) {
            alert("边已存在");
        } else {
            this.edge[v1][v2] = this.edge[v2][v1] = val;
            alert("添加成功");
        }
    },
    createEdge:function () {
                ...
                for(var j = 0;j<this.vexnum;j++){
                    if(i!=j && this.edge[i][j] != this.selfDis && this.edge[i][j] != this.infinite){
                        ...
                        $lines += '<div class="weight" onclick="changeDis('+i+','+j+')" id="v'+i+'v'+j+'" style="left: '+(x1+(x2-x1)/2)+'px;top: '+(y1+(y2-y1)/2)+'px;">'+this.edge[i][j]+'</div>\n' +
                            ...
                            '                        <line id="lv'+i+'lv'+j+'" x1="'+x1+'" y1="'+y1+'" x2="'+x2+'" y2="'+y2+'" stroke="#333333" stroke-width="2" marker-end="url(#arrow)"></line>\n' +
                            '                    </svg>';
    ...
    dijkstra:function (oV) { //original vertex
        ...
        $("#textWrapper").append('<div class="textItem" style="background-color:#F0FFFF">\n<div class="textItemLeft"></div>\n<div class="textItemRight">\n<h5>ending</h5>\n</div>\n<div class="clearfix"></div>\n</div>');
    },
    ...
    addEvent:function () {
        var that = this;
        $("#addEdgeb").click(function() {
            that.addEdge($("#vertex1").val().substring(1),$("#vertex2").val().substring(1),$("#value1").val());
            that.showEdges();
        });
